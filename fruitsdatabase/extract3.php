...
//ТУТ МЫ ЗАКОММЕНТИЛИ ЦИКЛ, КОТОРЫЙ ДЕЛАЕТ НАМ МАССИВ, КОТОРЫЙ НАМ НУЖЕН. пОСЛЕ Я УЗНАЛ, ЧО ТОЖЕ САМОЕ ДЕЛАЕТ ФЕТЧ_ОЛЛ
echo $i . '<br>' ;

var_dump($tableArray);
echo '<br>' ;
$tableArray = mysqli_fetch_all($data, MYSQL_ASSOC); //ПОЧЕМУ-ТО РУГАЕТСЯ НА АССОЦИАТИВНЫЙ МАССИВ, ПО УМОЛЧАНИЮ ХОЧЕТ НУМЕРОВАННЫЙ
var_dump($tableArray);
