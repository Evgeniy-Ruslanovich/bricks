...
function get_table_head($start_time, $end_time){
	$start_years = array( //даем массив соответствия стартового месяца стартовуму году
	1 => 2001,
	13 => 2002,
	25 => 2003,
	37 => 2004,
	...
	$end_years = array( //даем массив соответствия финального месяца финальному году - потом это можно бдует получать из базы
	12 => 2001,
	24 => 2002,
	36 => 2003,
	48 => 2004,
	...
	$table_head .= '<td>янв</td><td>фев</td><td>мар</td><td>апр</td><td>май</td><td>июн</td><td>июл</td><td>авг</td><td>сент</td><td>окт</td><td>ноя</td><td>дек</td>';
	}
...
function get_indicator_data($link, $indicator, $location, $sector, $start_time, $end_time) {
	...
	$values_q_result = mysql_query( $query, $link);

	/* это вариант для mysqli
	...
	*/
	$values_array = array();   // а это вариант для майскл старого. По сути что тут чтотам задача - перепихать из вертикальног массива в горизонтальный
	while ($row = mysql_fetch_row($values_q_result)) {
		array_push($values_array, $row[0]);
	}
